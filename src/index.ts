...
let tasks: Task[] = []; // Initializing an empty array to store tasks
...
const addTask = (id: number, args: Args) => {
  tasks.push({
    id,
    title: args.title || "No title",
    content: args.content || "",
    isDone: false,
  }); // Adding a new task object to the tasks array
...
const updateTask = (id: number, args: Args) => {
...
const showTasks = (id: number) => {
  if (id && id !== 0) {
    const task = tasks.find((task) => task.id === id); // Finding the task with the specified ID in the tasks array
    if (task) {
      console.log(
        `${title(task.title)}: ${
          task.isDone ? dim("completed") : dim("not completed")
        }`
      ); // Logging the title and completion status of the task to the console
      console.log(content(task.content)); // Logging the content of the task to the console with a black background
    } else {
      console.log(`Task with ID ${id} not found.`); // Logging an error message to the console if the task is not found
    }
  } else {
    ...
    tasks.forEach((task) => {
      ...
      const status = task.isDone ? "completed" : "not completed"; // Formatting the completion status of the task
      console.log(
